...
const Categories = () => {
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm(
        "Are you sure you want to delete this category? It might affect expenses using it."
      )
    ) {
      dispatch(categoryStart());
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted small mb-0">
            Manage your expense categories and keywords
          </p>
        </div>
                    ...
                    <div className="d-flex flex-wrap gap-1 mt-3">
                      {category.keywords.length > 0 ? (
                        category.keywords.slice(0, 5).map((kw, i) => (
                          <span
                            key={i}
                            className="badge bg-light text-muted fw-normal rounded-pill px-2"
                          >
                            {kw}
                          </span>
                        ))
                      ) : (
                        <span className="text-muted small italic text-opacity-50">
                          No keywords
                        </span>
                      )}

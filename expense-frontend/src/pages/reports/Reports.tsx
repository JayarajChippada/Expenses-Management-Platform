...
const Reports = () => {
  ...
  const fetchReportData = useCallback(
    async (tabIndex: number) => {
      ...
      try {
        ...
        switch (tabIndex) {
          case 0:
            response = await api.get(API_ENDPOINTS.REPORTS.OVERVIEW, {
              params: filters,
            });

            dispatch(reportsOverviewSuccess(response.data.data));

            response = await api.get(API_ENDPOINTS.REPORTS.TRENDS, {
              params: filters,
            });
          ...
          case 1:
            response = await api.get(API_ENDPOINTS.REPORTS.CATEGORIES, {
              params: filters,
            });
          ...
          case 2:
            response = await api.get(API_ENDPOINTS.REPORTS.MERCHANTS, {
              params: filters,
            });
  ...
  const fetchExportData = async () => {
    ...
    try {
      const [overviewRes, trendsRes, categoriesRes, merchantsRes] =
        await Promise.all([
      ...
      dispatch(
        reportsFailure(
          err.response?.data?.message || "Failed to fetch data for export"
        )
      );
  ...
  const handleExportPDF = async () => {
    ...
    const overviewData = [
      ...
      [
        "Savings Rate",
        `${
          data.overview.totalIncomes > 0
            ? Math.round(
                (data.overview.balance / data.overview.totalIncomes) * 100
              )
            : 0
        }%`,
      ],
    ...
    const merchantData = data.merchants
      .slice(0, 10)
      .map((m: any) => [m._id, `Rs. ${m.amount.toLocaleString()}`, m.count]);
  ...
  const handleExportExcel = async () => {
    ...
    const merchantsWS = XLSX.utils.json_to_sheet(
    ...
    const trendsData = data.trends.expenses
      .map((e: any) => ({
        Date: e._id,
    ...
    <div className="container-fluid p-0">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted small mb-0">
            Gain deep insights into your financial habits
          </p>
        </div>
        ...
        <div className="d-flex gap-2 align-items-center flex-wrap">
          <select
            ...
            onChange={(e) =>
              dispatch(setReportsFilters({ dateRange: e.target.value }))
            }
          >
      ...
      <div className="bg-white p-1 rounded-4 shadow-sm d-inline-flex mb-4 border border-light overflow-auto maxWidth-100">
        {tabs.map((tab, index) => (
          <button
            key={index}
            className={`btn border-0 fw-bold px-4 py-2 d-flex align-items-center gap-2 transition-all rounded-3 text-nowrap ${
              activeTab === index
      ...
      <div className="mt-2 position-relative">
        {loading && (
          <div
            className="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-white bg-opacity-75 z-index-10"
        ...
        <div style={{ visibility: loading ? "hidden" : "visible" }}>
          ...
          {activeTab === 1 && <ReportCategories />}

          {activeTab === 2 && <ReportMerchants />}
        </div>
      </div>
    </div>
  );

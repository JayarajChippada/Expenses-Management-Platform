...
const AddCategoryModal = ({ show, onClose, type = "expense" }: AddCategoryModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(categoryFailure(error.response?.data?.message || "Failed to create category"));
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="modal-content border-0 shadow-lg rounded-4">
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                ...
                <div className="mb-3">
                  <label className="form-label small fw-bold">Select Icon</label>
                  <div className="d-flex flex-wrap gap-2 p-3 bg-light rounded-3 border" style={{ maxHeight: '150px', overflowY: 'auto' }}>
                    {["📁", "💰", "🍕", "🚗", "🏠", "🎮", "👕", "🏥", "✈️", "💻", "🎬", "📚", "🏋️", "🎁", "📱", "💡", "🛠️", "🛒", "🚿", "🚆", "🚲", "🐾", "☕", "🍺", "🍷", "🍹", "🍰", "🍎", "🥦", "🥩"].map((emoji) => (
                      <button
                        key={emoji}
                        type="button"
                        className={`btn btn-sm ${formData.icon === emoji ? 'btn-primary shadow-sm' : 'btn-outline-light text-dark'}`}
                        style={{ fontSize: '1.2rem', width: '45px', height: '45px' }}
                ...
                <div className="mb-3">
                  <label className="form-label small fw-bold">Keywords (Optional)</label>
                  <input
                    type="text"
                    className="form-control rounded-3"

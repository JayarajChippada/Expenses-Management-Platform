    ...
    reader.onload = (e) => {
      try {
        ...
        const formattedData = rawData.map((row: any) => {
          const description =
            getValue(row, ["Description", "Desc"]) || "Imported Expense";
          const amount = parseFloat(
            getValue(row, ["Amount", "Amt", "Cost", "Price"]) || "0"
          );
          const categoryName =
            getValue(row, ["Category", "Category Name", "Cat"]) ||
            "Uncategorized";
          const merchant = getValue(row, ["Merchant", "Vendor", "Payee"]) || "";
          const paymentMethod =
            getValue(row, ["PaymentMethod", "Payment Method", "Method"]) ||
            "Cash";
          const dateVal = getValue(row, [
            "Date",
            "Txn Date",
            "Transaction Date",
          ]);
          ...
          <div className="modal-body p-4">
            <div className="mb-4">
              ...
              <div className="mt-2 extra-small text-muted">
                ...
                Expected columns: Date, Description, Amount, Category, Merchant,
                PaymentMethod
              </div>
            </div>
            ...
            {data.length > 0 && (
              <div>
                <h6 className="fw-bold text-dark mb-3 d-flex justify-content-between align-items-center">
                  ...
                  <span className="badge bg-primary-light text-primary-custom rounded-pill">
                    Validated
                  </span>
                </h6>
                <div
                  ...
                  <table className="table table-hover table-sm mb-0">
                    <thead className="bg-light sticky-top">
                      <tr>
                        ...
                        <th className="py-2 small fw-bold text-end pe-3">
                          Amount
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.slice(0, 10).map((row, idx) => (
                        <tr key={idx}>
                          ...
                          <td className="py-2 small fw-medium">
                            {row.description}
                          </td>
                          <td className="py-2 small text-end pe-3 fw-bold text-primary-custom">
                      ...
                      {data.length > 10 && (
                        <tr>
                          <td
                            colSpan={3}
                            className="text-center py-2 extra-small text-muted italic"
                          >

...
reportsService.getOverview = async (userId, dateRange) => {
  try {
    const { startDate, endDate } = getStartDateFromRange(dateRange) || {
    ...
    monthlyIncomes.forEach((i) => {
      if (!monthlyDataMap[i._id])
        monthlyDataMap[i._id] = { month: i._id, expenses: 0, income: 0 };
    ...
    return {
      ...
      incomeCount,
      avgExpense:
        expenseCount > 0 ? Math.round(totalExpenses / expenseCount) : 0,

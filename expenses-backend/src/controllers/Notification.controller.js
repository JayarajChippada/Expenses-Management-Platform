...
notificationController.fetchNotifications = async (req, res, next) => {
    try {
        ...
        console.log("Notification Controller fetchNotifications() method Error: ", error);
        next(error);
    }
};
...
notificationController.markAsRead = async (req, res, next) => {
    try {
        ...
        if (data) {
            res.status(200).json({ success: true, message: "Notification marked as read", data: data });
        } else {
...
notificationController.markAllAsRead = async (req, res, next) => {
    try {
        ...
        console.log("Notification Controller markAllAsRead() method Error: ", error);
        next(error);
    }
};
...
notificationController.deleteNotification = async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const notificationId = req.params.notificationId;
        const resObj = await notificationService.deleteNotification(userId, notificationId);
        ...
        console.log("Notification Controller deleteNotification() method Error: ", error);
        next(error);
    }
};

...
reportsController.getCategoryBreakdown = async (req, res, next) => {
  try {
    ...
    console.log(
      "Reports Controller getCategoryBreakdown() method Error: ",
      error
    );
    next(error);
  }
...
reportsController.getMerchantBreakdown = async (req, res, next) => {
  try {
    ...
    console.log(
      "Reports Controller getMerchantBreakdown() method Error: ",
      error
    );
    next(error);
  }
...
reportsController.exportReport = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: `Report export in ${format} format is not yet fully implemented, but the endpoint is ready.`,
      });
  } catch (error) {
